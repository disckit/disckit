...
class AntifloodManager {
  ...
  constructor(options = {}) {
    ...
    this._statsHits        = 0;
    this._statsBlocked     = 0;
    this._statsWhitelisted = 0;
  }
  ...
  /**
   * Returns lifetime counters for this manager instance.
   * @returns {{ hits: number, blocked: number, whitelisted: number, ratio: number }}
   */
  stats() {
    const total = this._statsHits + this._statsBlocked;
    return {
      hits:        this._statsHits,
      blocked:     this._statsBlocked,
      whitelisted: this._statsWhitelisted,
      ratio:       total === 0 ? 0 : Math.round((this._statsBlocked / total) * 100) / 100,
    };
  }

  /** Reset lifetime stats counters without clearing flood state. */
  resetStats() {
    this._statsHits        = 0;
    this._statsBlocked     = 0;
    this._statsWhitelisted = 0;
