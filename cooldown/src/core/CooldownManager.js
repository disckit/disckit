...
class CooldownManager {
  ...
  /**
   * Checks cooldowns for multiple commands at once for the same key.
   * Returns the first cooldown that is still active, or `{ ok: true }` if all passed.
   * Does NOT apply any cooldown — use `check()` for each individually after this.
   *
   * @param {string[]} commands  List of command names to check.
   * @param {string}   key       User ID, guild ID, or any string identifier.
   * @returns {{ ok: boolean, command?: string, remaining: number, remainingText: string, expiresAt: number }}
   *
   * @example
   * const result = manager.checkMany(["ban", "kick"], userId);
   * if (!result.ok) {
   *   reply(`⏳ Aguarde ${result.remainingText} antes de usar \`${result.command}\` novamente.`);
   * }
   */
  checkMany(commands, key) {
    if (this._bypass.has(key)) return { ok: true, remaining: 0, remainingText: "0s", expiresAt: 0 };
    const now = Date.now();
    for (const command of commands) {
      ...
      if (expiresAt > now) {
        const remaining = expiresAt - now;
        return { ok: false, command, remaining, remainingText: formatMs(remaining), expiresAt };
      }
    }
    return { ok: true, remaining: 0, remainingText: "0s", expiresAt: 0 };
  }
