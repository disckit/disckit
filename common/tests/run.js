...
describe("ESM wrapper — smoke test (v1.3.0)", () => {
  // Verify the ESM wrapper file exists and is loadable via CJS require (simulates the import path)
  it("esm/index.js exists",       () => { const fs = require("node:fs"); const p = require("node:path"); assert.ok(fs.existsSync(p.join(__dirname, "../esm/index.js"))); });
  it("esm/package.json has type module", () => { const p = require("../esm/package.json"); assert.strictEqual(p.type, "module"); });
  it("package.json has module field",    () => { const p = require("../package.json"); assert.ok(p.module); });
  it("package.json exports import key",  () => { const p = require("../package.json"); assert.ok(p.exports["."].import); });
  it("package.json exports require key", () => { const p = require("../package.json"); assert.ok(p.exports["."].require); });
  it("package.json exports types key",   () => { const p = require("../package.json"); assert.ok(p.exports["."].types); });
  it("version is 1.3.0",                () => { const p = require("../package.json"); assert.strictEqual(p.version, "1.3.0"); });
});
