...
/**
 * Format a date or timestamp as a Discord timestamp tag.
 *
 * @param {Date|number} date  Date object or Unix timestamp in milliseconds.
 * @param {"t"|"T"|"d"|"D"|"f"|"F"|"R"} [format="f"]
 *   Discord timestamp format:
 *   - `t` → short time      (e.g. 9:41 PM)
 *   - `T` → long time       (e.g. 9:41:30 PM)
 *   - `d` → short date      (e.g. 11/28/2018)
 *   - `D` → long date       (e.g. November 28, 2018)
 *   - `f` → short date/time (e.g. November 28, 2018 9:41 PM)
 *   - `F` → long date/time  (e.g. Wednesday, November 28, 2018 9:41 PM)
 *   - `R` → relative time   (e.g. 3 years ago)
 * @returns {string} e.g. "<t:1543363260:R>"
 *
 * @example
 * toDiscordTimestamp(new Date(), "R")    // → "<t:1711234567:R>"
 * toDiscordTimestamp(Date.now(), "D")    // → "<t:1711234567:D>"
 * toDiscordTimestamp(member.joinedAt)    // → "<t:1711234567:f>"
 */
